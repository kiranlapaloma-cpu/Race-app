...
def parse_race_time(val):
    """
    Accepts seconds (73.30), 'M:SS.ms', or vendor numerics:
      - centiseconds (e.g., 7330 -> 73.30s)
      - milliseconds (e.g., 73300 -> 73.300s)
...
def normalize_time_columns(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    ...
    for c in cols:
        ...
        med = out[c].median()
        if 100 <= med < 20000:        # likely centiseconds for 100–400m windows
            out[c] = out[c] / 100.0
        elif 10000 <= med < 200000:   # milliseconds fallback
            out[c] = out[c] / 1000.0
    return out
...
# =================================
# Distance-aware PI weights (v2.4)
# =================================
...
def build_manual_template(distance_m: int, n_horses: int) -> pd.DataFrame:
    ...
    for i in range(1, n_horses+1):
        ...
        base[f"H{i}_Time"] = np.nan  # segment time (s) for each 200m step
    return base

def manual_compute_longform_to_wide(man_df: pd.DataFrame, horse_names: list, distance_m: int) -> pd.DataFrame:
    """
    Convert manual 200m long grid into wide schema the app expects.
    """
    df = man_df.copy()
    out_rows = []
    for idx, name in enumerate(horse_names, start=1):
        ...
        # helper: sum times from start_m down to end_m (exclusive), 200m steps
        ...
        out_rows.append(dict(
            ...
            **{
                ...
                "200_Pos": pos_200,
                "400_Pos": pos_400,
                # 100m split placeholders (not available in manual)
...
def build_sectionals(df: pd.DataFrame, distance_m: float) -> pd.DataFrame:
    ...
    # race time (seconds) — ALWAYS recompute from "Race Time" string if present
    ...
    # normalise vendor-style units on window columns
    ...
    # ---- F200 from the START (dynamic columns if present) ----
    ...
    if c1 and c2:
        ...
        out["F200"] = np.nan  # manual mode without explicit first 200
...
def compute_PI(df_sec: pd.DataFrame, distance_m: float) -> pd.DataFrame:
    ...
    # robust z + logistic mapping to 0–10
...
def compute_GPI(df_sec_pi: pd.DataFrame, distance_m: float) -> pd.DataFrame:
    ...
    # stress: above medians on all four
    meds = {
    ...
    winner_time = out.loc[out["Finish_Pos"].astype("Int64").idxmin(), "RaceTime_s"] if "Finish_Pos" in out.columns and out["Finish_Pos"].notna().any() else np.nan
...
try:
    ...
    # numeric casting for window columns
    for c in ["800-400", "400-Finish", "100_Time", "200_Time", "300_Time", "Finish_Time", "Finish_Pos"]:
        if c in work.columns:
            work[c] = pd.to_numeric(work[c], errors="coerce")

    # race time normalisation is done inside build_sectionals
...
# Visuals
# -------------
# 1) Sectional Shape Map — Kick vs Grind
...
# 2) Pace Curves — Average + Top 8 by finish
...
st.caption(
    ...
    "sanity anchors to estimate group potential (0–10)."
)
